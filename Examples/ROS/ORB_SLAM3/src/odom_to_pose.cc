...
{
  odom_sub_ = nh_.subscribe("/terrasentia/mhe_odom", 100, &OdomToPose::odomCallback, this);
  pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("/terrasentia/mhe_pose", 100);
  nhp_.param("target_frame", target_frame, std::string());
}

void OdomToPose::odomCallback(const nav_msgs::Odometry::ConstPtr& odom_msg)
{
  //don't really need covariance
  ...
  /*
  //not too sure what this line onwards does
